...
# "/Users/localadmin/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Documents/skinner/data/prolific/clock_v2_pilot/new_cotingencies_05-19-23/processed/all_processed_data_2023-05-22.csv"
...
car::Anova(m2_1, '3')
# 
# m3 <- lmer(pos_shifted ~ scale(vmax_location)*scale(vmax) + scale(att_location)*scale(vmax) + (1|subject), 
#            df)
# summary(m3)
# car::Anova(m3, '3')
...
# alternative thought: use the last location
mlist <- list()
for (i in 1:1000) {
...
ggplot(df, aes(vmax_theta - resp_theta, ev, color = vmax > 166)) + geom_point()
...
br.formula <- brmsformula(resp_theta_c ~ vmax_theta_c + vmax_theta_c + 
                            ...
                            (1|subject), kappa ~ reward_lag) + von_mises(link_kappa = "log")
...
prior_fit_von <- c(get_prior(br.formula, df1, family = "von_mises") %>% filter(!class == "b"),
...
bm12 <- update(bm12, formula = br.formula, newdata = df1, prior = prior_fit_von, chains = 4, iter = 2000, backend = "cmdstanr", threads = threading(4),
...
# do we need to enter sin and cos for circular predictors?
